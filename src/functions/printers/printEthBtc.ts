...
export const printEthBtc = (response: IResponse, wallet: TWallet) => {
  ...
  const { ETH, BTC } = wallet;

  const currentEthValue = (eth * ETH).toFixed(2);
  const currentBtcValue = (btc * BTC).toFixed(2);
  const balance = (eth * ETH + btc * BTC).toFixed(2);
  const calculatedEthBtc = (eth / btc).toFixed(10);
  const diff = (eth / btc - ethBtc).toFixed(10);

  const ethToBtcDiff = (ETH * ethBtc * btc - Number(currentEthValue)).toFixed(
    2
  );
  const ethToBtc = {
    amount: (ETH * ethBtc).toFixed(6),
    value: (ETH * ethBtc * btc).toFixed(2),
    diff: ethToBtcDiff,
  };

  const btcToEthDiff = ((BTC / ethBtc) * eth - Number(currentBtcValue)).toFixed(
    2
  );
  const btcToEth = {
    amount: (BTC / ethBtc).toFixed(5),
    value: ((BTC / ethBtc) * eth).toFixed(2),
    diff: btcToEthDiff,
  };
